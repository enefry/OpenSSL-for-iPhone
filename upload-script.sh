...
RELEASE_ID=`./github_release.sh list_releases "${GITHUB_ACTOR}" "${repository_name}" | grep "${versionTag}" | awk '{print $3}'`

if [ "${RELEASE_ID}" = "" ];
then
    ./github_release.sh create_release "${GITHUB_ACTOR}" "${repository_name}" "${versionTag}"
    RELEASE_ID=`./github_release.sh list_releases "${GITHUB_ACTOR}" "${repository_name}" | grep "${versionTag}" | awk '{print $3}'`
fi
